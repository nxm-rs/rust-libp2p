//! Private-to-private WebRTC transport.
//!
...
//! connection, then both sides run a full ICE agent to open a direct connection. The
//! DTLS handshake authenticates the remote against the certificate fingerprint carried
//! in the exchanged SDP; the peer identity is the one authenticated on the relayed
//! connection, so no further handshake runs on the direct connection.
...
use crate::tokio::{connection::Connection, error::Error};

/// Configuration of the private-to-private WebRTC transport.
#[derive(Debug, Clone)]
pub struct Config {
    /// ICE server urls handed to the ICE agent, e.g. `stun:stun.example.net:3478`.
...
impl libp2p_core::Transport for Transport {
    ...
    fn listen_on(
        ...
        let incoming = self
            .incoming
            .take()
            .ok_or(TransportError::Other(Error::Internal(
...
mod tests {
    ...
    fn dial_addr_roundtrip() {
        ...
        let addr: Multiaddr = format!(
            "/ip4/127.0.0.1/tcp/4001/p2p/12D3KooWGQTQuV6JfgpKpc847NMsxaFnKXDEpkN5kbeH1REW41BR/p2p-circuit/webrtc/p2p/{target}"
        )
        .parse()
        .unwrap();
